...
final class HolidayResolver
{
     ...
     *
     * NOTE:
     * - All names MUST exactly match FPP UI labels
     * - Rules are deterministic and reversible
     */
    private const HOLIDAYS = [
        ...
        self::LOCALE_USA => [
            ...
            // Easter-based (offsets from Easter Sunday)
    ...
    /**
     * Resolve a holiday name to a DateTime.
     */
    public static function dateFromHoliday(
        ...
        $rules = self::HOLIDAYS[$locale][$holiday] ?? null;
        if (!$rules) {
            return null;
        }

        return self::resolveRule($holiday, $rules, $year, $locale);
    }

    /**
     * Resolve all holidays that fall on a given date.
     *
     * @return string[] holiday names
     */
    ...
    private static function resolveRule(
        ...
        switch ($rule['type']) {
            ...
            case 'weekday':
                return self::nthWeekdayOfMonth($year, $rule['month'], $rule['weekday'], $rule['nth']);

            case 'weekday_last':
                return self::lastWeekdayOfMonth($year, $rule['month'], $rule['weekday']);

            case 'offset':
                $base = self::dateFromHoliday($rule['base'], $year, $locale);
                return $base ? (clone $base)->modify(($rule['days'] >= 0 ? '+' : '') . $rule['days'] . ' days') : null;
    ...
    {
        return (new DateTime())->setTimestamp(easter_date($year))->setTime(0, 0, 0);
    }
